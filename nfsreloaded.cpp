...
private:
	...
	int rxb,rxe,ry1b,ry2b,ry3b,ry4b,ry1e,ry2e,ry3e,ry4e; 	//coordinates of rectangles in game screen
	int currentj;
	int escp,pausep;
public:
	game();
...
void game::play()
{
	...
	for (int i=0;i<=75;i++)
	{
		...
		bar(getmaxx()/2-88,getmaxy()-28,getmaxx()/2-52+i*2,getmaxy()-22);
		delay(15);
	}
	setcolor(0);
	outtextxy(maxx/2-90,maxy-80,"Loading.....") ;
	...
	do
	{
		for(int i=1;i<=15&&life>0&&complete==0;i++)
		{
			...
			int y1=0;int x1=c1+(random(3)*50);
			int y2=0;int x2=c1+(random(3)*50);
			...
			do
			{
				...
				if(x==x1||x==x2)
				{
					if((((y+80)-(y1-20))>1 && ((y+80)-(y1-20))<199) || (((y+80)-(y2-20))>1 && ((y+80)-(y1-20))<199)  )
					{
					accident=1;
					complete=1;
					}
	...
	if(currentplayer.level==7&&life>0)
	{
		...
		outtextxy(55,230,"Press <Esc.> to exit to main menu...");int ch;
		do
		{
		  ch=getch();
		  if(ch==49)
...
void game::highscores()
{
	...
	gotoxy(53,9+(i*2));cout<<x.score;
	gotoxy(72,9+(i*2));cout<<x.level;
    }
    setcolor(YELLOW);
